...
def entries_histogram(turnstile_weather):
    ...
    # your code here to plot a histogram for hourly entries when it is not
    # raining
...
def mann_whitney_plus_means(turnstile_weather):
    ...
    # YOUR CODE HERE
    with_rain = turnstile_weather['ENTRIESn_hourly'][
        turnstile_weather.rain == 1]
    without_rain = turnstile_weather[
        'ENTRIESn_hourly'][turnstile_weather.rain == 0]
...
def normalize_features(df):
    ...
    if (sigma == 0).any():
        raise Exception(
            "One or more features had the same value for all samples, and thus could " +
...
def compute_cost(features, values, theta):
    ...
    # your code here
    m = len(values)
    cost = 1 / (2 * m) * ((values - np.dot(features, theta)) ** 2).sum()
    return cost
...
def predictions(dataframe):
    ...
    theta_gradient_descent, cost_history = gradient_descent(
        features_array, values_array, theta_gradient_descent, alpha, num_iterations)
...
def plot_cost_history(alpha, cost_history):
    ...
    return ggplot(cost_df, aes('Iteration', 'Cost_History')) + geom_point() + \
        ggtitle('Cost History for alpha = %.3f' % alpha)
...
def plot_residuals(turnstile_weather, prediction):
    ...
    plt.figure()
    # Your code goes here:
    (turnstile_weather['ENTRIESn_hourly'] - prediction).hist()
    return plt
...
def compute_r_squared(data, prediction):
    ...
    # your code here
    r_squared = 1 - sum((data - prediction) ** 2) / \
        sum((data - np.mean(data)) ** 2)
    return r_squared
