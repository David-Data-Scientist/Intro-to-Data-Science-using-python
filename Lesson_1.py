...
def avg_medal_count2():
    ...
    Before we start, here are some useful code to apply:
    df.apply(np.mean)  # get the mean for each column
    df['one'].map(lambda x: x>=1) # select 'one' column that x is greater and equal to 1
    df.applymap(lambda x: x>=1) # for every column, select x is greater and equal to 1 in the data frame
...
def numpy_dot():
    ...
    # YOUR CODE HERE
    point = np.dot([4, 2, 1], [gold, silver, bronze])  # 1*3 by 3*n matrix
    olympic_points_df = DataFrame({'country_name': Series(countries), 'points': Series(point)})
